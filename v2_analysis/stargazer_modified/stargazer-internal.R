...
.stargazer.wrap <-
  ...
  .iround <- 
    function(x, decimal.places=0, round.up.positive=FALSE, simply.output=FALSE) {
      ...
      # GC added scientific notation for very small numbers
      use_scientific = round.result < 0.001
      round.result.char <- as.character(format(round.result, scientific=use_scientific))
